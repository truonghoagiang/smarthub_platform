...
create or replace function f_get_devices_by_user_and_device_type(i_user_id in int, i_device_type_id in int)
returns table (
		id int,
		brand_name varchar,
		auth_type varchar,
...
$$
		begin
			return QUERY select td.id, bb.brand_name, tb.name, tt.type_name, rr.room_name, td.is_online, td.is_active, td.capbilities, td.current_statem,
						...
						where td.device_type = tt.type_id
						  and td.brand_id = bb.brand_id 
						  and td.room_id = rr.id
						  and td.user_id = i_user_id
						  and td.device_type = i_device_type_id;
...
--select * from t_brand_connection tbc ;